...
def get_dataset_loader(args, name, batch_size, num_frames, split='train', hml_mode='train'):
    if name != 'babel':
        ...
        from data_loaders.multi_motion.data.dataset import BABEL
        datapath = './data/babel/babel-smplh-30fps-male'
        framerate = 30
        ...
        rifke = Rifke(jointstype='mmm', forward_filter=False,
            path='./deps/transforms/joints2jfeats/rifke/babel-amass',
            normalization=True
            )
        bs = 128 if split == 'val' else 16
        smplh = SMPLH(path='./data/smpl_models/smplh',
            jointstype='mmm', input_pose_rep='matrix', batch_size=bs, gender='male')
        globalvelandy = Globalvelandy(canonicalize=True, pose_rep='rot6d', offset=True,
            path='./deps/transforms/rots2rfeats/globalvelandy/rot6d/babel-amass',
            # path='./deps/transforms/rots2rfeats/globalvelandy/rot6d/babel-amass/separate_pairs',
            normalization=True)
        transforms = SMPLTransform(rots2rfeats=globalvelandy, rots2joints=smplh,
            joints2jfeats=rifke)
        frame_sampler = FrameSampler()
        frame_sampler.max_len = args.max_len
